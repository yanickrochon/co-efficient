...
function * processPartialRenderer(compiledData, segValue, segKey, segments) {
  return 'yield(' + OBJ_ENGINE + '.stream)(' + quote(segValue.literal) +
    ',' + context(segValue) + ',b);';
}
...
function modifier(str, modifiers) {
  if (modifiers) {
    return OBJ_ENGINE + '.modifiers.' + modifiers.split('').join('(' + OBJ_ENGINE + '.modifiers.') +
           '(' + str + repeat(')', modifiers.length);
  }
...
/**
Source: http://stackoverflow.com/questions/202605/repeat-string-javascript
*/
