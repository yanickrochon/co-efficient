...
processBlockContextObj = {
  ...
  //OBJ_PARAMS: FN_PARAMS,       // not required, because of processParams
...
function registerBlockRenderer(id, renderer) {
  if (typeof id !== 'string' && id.length !== 1) {
    throw CompilerException('Invalid block identifier');
  }
  if (BLOCK_RENDERERS[id] || !VALID_BLOCK_IDENTIFIERS.test(id)) {
    throw CompilerException('Illegal block identifier');
  }
  if (!(renderer instanceof Function)) {
    throw CompilerException('Invalid renderer function');
  }
...
function * processBlock(compiledData, segValue, segKey, segments) {
  if (segValue.type) {
    ...
    throw CompilerException('Unknown block', segValue);
  }
  return false;
}
...
function * processContext(compiledData, segValue, segKey, segments) {
  if (!segValue.type && segValue.context) {
    if (!compiledData.renderers[CTX_RENDERER]) {

      compiledData.renderers[CTX_RENDERER] = 'function ' + CTX_RENDERER + '(' + OBJ_CONTEXT + '){' +
          OBJ_CONTEXT + '&&(' + OBJ_CONTEXT + '.data!==null&&' + OBJ_CONTEXT + '.data!==undefined)&&' + OBJ_STREAM + '.write(' + OBJ_CONTEXT + '.data);' +
        '}';
    }

    return modifier(CTX_RENDERER + '(' + context(segValue.context) + ');', segValue);
  }
  return false;
}
...
function * processInlineBlockDeclare(compiledData, segValue, segKey, segments) {
  compiledData.inlineBlocks[segValue.name] = OBJ_BLOCKS + '[' + quote(segValue.name) + ']=' + (yield processRenderer(compiledData, segValue, segKey, segments, true)) + ';';
  return '';
}
function * processInlineBlockRenderer(compiledData, segValue, segKey, segments) {
  var renderVal = 'yield(' + OBJ_BLOCKS + '[' + quote(segValue.name) + ']' +
    '(' + context(segValue.context) + ').render)(0)';

  if (Compiler.IGNORE_MISSING_INLINE_BLOCKS) {
    renderVal = OBJ_BLOCKS + '[' + quote(segValue.name) + ']&&(' + renderVal + ');';
  } else {
    renderVal = 'if(' + OBJ_BLOCKS + '[' + quote(segValue.name) + ']){' + renderVal + ';}else{throw RenderException("Missing block `' + segValue.name + '`");};';
  }
...
function * processPartialRenderer(compiledData, segValue, segKey, segments) {
  return modifier('yield(' + OBJ_ENGINE + '.stream)(' + quote(segValue.literal) +
    ',' + context(segValue.context) + ',' + OBJ_BLOCKS + ');', segValue);
}
...
function * processRenderer(compiledData, segValue, segKey, segments, contextSwitchable) {
  ...
  if (rendererStrArr.length) {
    rendererKey = (FN_RENDERER + segKey);
