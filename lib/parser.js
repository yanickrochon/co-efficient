...
const BLOCK_RULES = {
  ...
  '>': {   // partial renderer
    openingContent: 'inBlock',
    validContent: { 'literal': true, 'context': true, 'params': true },
    maxSiblings: false,
    selfClosing: true,
    closeBlock: false
  },
...
var ID_POOL = Date.now();   // init this at startup, increment as needed
...
function * parseString(str) {
  ...
  var state = {
    ...
    //   {openBlock{"inBlock+inQuote":inContext inParam=inParamValue inParam="inParamValue+inQuote"/}closeBlock}
    //
...
function * readBlock(state) {
  ...
  function joinSegment() {
    ...
    } else if ((maxSiblings = BLOCK_RULES[state.segment.type].maxSiblings) === false) {
    ...
    // count siblings
    for (var s = state.segment.segments, i = 0, len = s.length; i < len; ++i) {
      if (s[i].id === headSegment || s[i].headSegment === headSegment) {
        ++curSiblings;
      }
    }
    ...
    switch (c = state.nextChar) {
      ...
      case '~':
        if (state.escaped || state.inQuote) {
          state.escaped = false;
          state.pushBuffer(c);
        } else if ((state.inBlock || state.inContext) && !state.hasBufferContent) {
          saveSegmentLength();
          joinSegment();
        } else {
          throw new ParseException('Unexpected ~', state, true);
      ...
      case ':':
        if (state.escaped || state.inQuote) {
          state.escaped = false;
          state.pushBuffer(c);
        } else if (state.inBlock || state.segment.literal) {
          flushName();
          state.inContext = true;
        } else {
          throw new ParseException('Unexpected :', state, true);
      ...
      case '/':
        if (state.escaped || state.inQuote) {
          ...
          if (!(state.hasBufferContent || state.segment.literal || state.segment.name || state.segment.context)) {  // close block
            if (!BLOCK_RULES[state.segment.type].closeBlock || state.nextChar !== '}' || state.nextChar !== '}') {
              throw new ParseException('Unexpected /', state, true);
            }
            discardSegment();
            saveSegmentLength();
            ...
            if (!BLOCK_RULES[state.segment.type].selfClosing) {
              throw new ParseException('Unexpected /', state, true);
            }
          }
